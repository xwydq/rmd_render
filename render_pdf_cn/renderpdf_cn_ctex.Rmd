...
```{r scatter, dev="png", fig.cap='cars数据散点图以及回归直线。'}
# par(family='NSimSun')
par(mar = c(4, 4, .1, .1), las = 1)
plot(cars, pch = 19, main='中文显示')
abline(fit, col = 'red')
```

```{r ggplt, dev="png", fig.cap='ggplot画图'}
par(mar = c(4, 4, .1, .1), las = 1)
plot(cars, pch = 19, main='中文显示')
abline(fit, col = 'red')
```
...
ds <- df %>%
  ...
  # plyr::ddply(df, "gp", plyr::summarise, mean = mean(y), sd = sd(y))
